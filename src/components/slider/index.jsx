...
class Slider extends React.Component {
  ...
  next() {
    ...
    if( active > this.props.children.length - 2)
      active = 0;
    else
      ++active;
    this.goTo(active);
    ...
    return (
              <div className={classes}>
                <If condition={this.state.showArrows}>
                  <div className="slider-arrow slider-arrow-left" onClick={this.previous}>
                      <Icon name="chevron_left" size={this.state.arrowSize}/>
                  </div>
                  <div className="slider-arrow slider-arrow-right"  onClick={this.next}>
                      <Icon name="chevron_right" size={this.state.arrowSize}/>
                  </div>
                </If>
                <Draggable
                  axis="x"
                  ...
                  ref={c => { this.draggableComponent = c}}
                >
                  <div className="slider-slide-wrapper" style={style} ref={c => { this.draggableContent = c}}>
                    {
                      this.props.children.map((child, index) => {
                        ...
                        return (
                            <div style={slideStyle} key={index} ref={c => { this.slides[index] = c}}>
                              <If condition={shouldRenderChild || this.state.alreadyLoaded[index]}>
                                {child}
                              </If>
                            </div>
                          )
