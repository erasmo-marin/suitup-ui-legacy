  ...
  render() {
    ...
    return React.createElement(
      ...
      this.state.showArrows ? [React.createElement(
        'div',
        { className: 'slider-arrow slider-arrow-left', onClick: this.previous, key: '0'
        },
        React.createElement(Icon, { name: 'chevron_left', size: this.state.arrowSize })
      ), React.createElement(
        'div',
        { className: 'slider-arrow slider-arrow-right', onClick: this.next, key: '1'
        },
        React.createElement(Icon, { name: 'chevron_right', size: this.state.arrowSize })
      )] : null,
      React.createElement(
        Draggable,
        {
        ...
        React.createElement(
          'div',
          { className: 'slider-slide-wrapper', style: style, ref: c => {
          ...
          this.props.children.map((child, index) => {
            ...
            let shouldRenderChild = !this.state.lazyLoad || this.state.lazyLoad && Math.abs(this.state.activeIndex - index) < 2;

            if (shouldRenderChild && !this.state.minimalRender) {
              this.state.alreadyLoaded[index] = true;
            }

            return React.createElement(
              'div',
              { style: slideStyle, key: index, ref: c => {
                  this.slides[index] = c;
                } },
              shouldRenderChild || this.state.alreadyLoaded[index] ? child : null
            );
          })
        )
      ),
      this.state.showDots ? React.createElement(
        'div',
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            this.props.children.map((child, index) => {
              ...
              return React.createElement(
                Box.Child,
                { key: index, onClick: () => this.goTo(index), wide: 1 },
